...
export default function YearlyGoalsPage() {
  const { data: goals, isLoading } = trpc.goal.getAll.useQuery({ type: "YEARLY" });

  return (
    <div className="space-y-6 animate-fade-in">
      <h1 className="text-2xl font-bold tracking-tight">Yearly Goals</h1>
      {isLoading ? (
                  ...
                  <CardContent className="p-4">
                    ...
                    {goal.childGoals && goal.childGoals.length > 0 && (
                      <p className="text-xs text-muted-foreground mt-2">
                        {goal.childGoals.length} monthly sub-goals
                      </p>
                    )}
                  </CardContent>
                </Card>
        ...
        <p className="text-muted-foreground">No yearly goals set.</p>
      )}
    </div>
  );
}
